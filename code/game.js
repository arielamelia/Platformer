/*
like the lava function, repeat 
constructor identity for bullet, start at player position at key press
*/
...
function Shard(pos) {
  ...
  this.size = new Vector(1.25, 1.25);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
Shard.prototype.type = "shard";

// Lava is initialized based on the character, but otherwise has a
// size and position

function Spikel(pos, ch){
	this.pos = pos;
	this.size = new Vector (1,1);
}
Spikel.prototype.type="spikel";

function Spiker(pos, ch){
	this.pos = pos;
	this.size = new Vector (1,1);
}
Spiker.prototype.type="spiker";

function Spiket(pos, ch){
	this.pos = pos;
	this.size = new Vector (1,1);
}
Spiket.prototype.type="spiket";
...
Spikel.prototype.act = function(step, level){
 var newPos = this.pos;
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  
};
Spiker.prototype.act = function(step, level){
 var newPos = this.pos;
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  
};
Spiket.prototype.act = function(step, level){
 var newPos = this.pos;
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
...
Shard.prototype.act = function(step) {
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    this.finishDelay = 3.5;
	  ...
	  this.status="lost";
	  this.finishDelay = 1;  
  }else if (type == "spikel" || type == "spiker" || type == "spiket"&& this.status == null){
	this.status = "lost";
	this.finishDelay = 1;	
  }   else if (type == "shard") {
...
var arrowCodes = {37: "left", 38: "up", 39: "right", 88: "shoot"};
  ...
  function startLevel(n) {
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost")
        ...
        var source = document.getElementById("endScreen")
		var image = document.createElement('img');
		source.appendChild(image);
		image.src = "css/art/winning.gif"
    });
  }
  startLevel(0);
}
